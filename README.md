...
每次查询完成后等待 60 秒再检查（接口请求耗时另计）。AstrBot 日志按 `Tracker monitor` 搜索，INFO 级别会显示监控是否启用、每次查询的直播状态、首次基线或状态未变化。配置错误、接口错误及通知失败以 WARNING 输出。

若先在 OBS 开播、再加载插件，首次检查只记录“正在直播”，不会补发。可停止推流并等待日志显示“未开播”，再开始推流；下次成功检查应发送开播通知。测试过程中不要重新加载插件，否则会重新建立基线。
